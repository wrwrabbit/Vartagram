        ...
        |> mapToSignal { [weak self] query, approvedGlobalPostQueryState, options, searchScope, downloadItems, _, _ -> Signal<([ChatListSearchEntry], Bool, String?)?, NoError> in
            ...
            if case .savedMessagesChats = location {
                ...
                if query.hasPrefix("#") {
                    ...
                    let updatedLocalPeers = context.engine.contacts.searchLocalPeers(query: query.lowercased(), inactiveSecretChatPeerIds: context.inactiveSecretChatPeerIds)
...
//                    addAppLogEvent(postbox: context.account.postbox, type: "search_global_query")
                ...
                if key == .globalPosts {
                    ...
                    searchSignals = searchLocations.map { searchLocation in
                        return context.engine.messages.searchMessages(location: searchLocation, query: finalQuery, state: nil, limit: 50, inactiveSecretChatPeerIds: context.currentInactiveSecretChatPeerIds.with { $0 })
                    }
                }
            ...
            if case let .forum(peerId) = location, (key == .topics || key == .chats) {
                foundThreads = chatListViewForLocation(chatListLocation: location, location: .initial(count: 1000, filter: nil), account: context.account, shouldLoadCanMessagePeer: true, inactiveSecretChatPeerIds: context.inactiveSecretChatPeerIds)
