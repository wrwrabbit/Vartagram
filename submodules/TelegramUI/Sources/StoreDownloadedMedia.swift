...
private final class DownloadedMediaStoreManagerPrivateImpl {
    ...
    var mediaStoreAllowed: Bool = false
...
final class DownloadedMediaStoreManagerImpl: DownloadedMediaStoreManager {
    ...
    private var mediaStoreAllowedDisposable: Disposable?
    
    init(postbox: Postbox, accountManager: AccountManager<TelegramAccountManagerTypes>, mediaStoreAllowed: Signal<Bool, NoError>) {
        let queue = self.queue
        self.postbox = postbox
        self.impl = QueueLocalObject(queue: queue, generate: {
            return DownloadedMediaStoreManagerPrivateImpl(queue: queue, postbox: postbox, accountManager: accountManager)
        })
        self.mediaStoreAllowedDisposable = mediaStoreAllowed.start(next: { [weak self] value in
            self?.impl.with { impl in
                impl.mediaStoreAllowed = value
            }
        })
    }
